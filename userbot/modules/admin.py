...
from telethon.errors.rpcerrorlist import MessageTooLongError, UserIdInvalidError, MessageNotModifiedError, UserAdminInvalidError
...
from telethon.tl.types import (
    ChannelParticipantCreator as ParticipantCreator,
    ChannelParticipantAdmin as ParticipantAdmin,
...
@register(outgoing=True, pattern=r"^\.rocker(?: |$)(.*)")
async def rocker(event):
    ...
    if not admin and not creator:
        await event.edit("`#Disclaimer ❌\nThis plugin is specifically for Owner and Co-Founders.`")
        return
    if rockers is True:
        await event.delete()
    else:
    ...
    async for user in event.client.iter_participants(event.chat_id):
        if user.id == lynxget.id:
            pass
        try:
            if not (isinstance(user.participant, ParticipantAdmin) or isinstance(user.participant, ParticipantCreator)):
                crying = await event.client(EditBannedRequest(event.chat_id, int(user.id), ChatBannedRights(until_date=None, view_messages=True)))
...
@register(outgoing=True, pattern=r"^\.gohell(?: |$)(.*)")
